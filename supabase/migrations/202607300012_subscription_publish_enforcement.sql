-- Require webhook-backed marketplace access for publishing.
...
begin
 ...
 if not exists(select 1 from profiles p join provider_profiles pp on pp.user_id=p.id
   ...
   and exists(select 1 from provider_subscriptions ps where ps.user_id=auth.uid() and ps.status in('trialing','active'))) then raise exception using errcode='42501',message='provider_unapproved'; end if;
...
begin
 ...
 if next_status='published' and not exists(select 1 from provider_subscriptions ps where ps.user_id=auth.uid() and ps.status in('trialing','active')) then raise exception using errcode='42501',message='billing_required'; end if;
